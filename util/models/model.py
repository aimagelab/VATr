...
class Generator(nn.Module):
    ...
    def Eval(self, ST, QRS):
        ...
        if IS_SEQ:
            B, N, R, C = ST.shape
            FEAT_ST = self.Feat_Encoder(ST.view(B * N, 3, R, C))
    ...
    def visualize_attention(self):
        ...
        for sim_, sle_ in zip(simgs, slen):
            ...
            for pi in range(sim_.shape[1] // sim_.shape[0]):
                STdict[count] = {'patch': sim_[:, pi * 32:(pi + 1) * 32], 'ischar': sle_ >= pi * 32,
                                 'encoder_attention_score': selfatt[count], 'decoder_attention_score': decatt[:, count]}
                count = count + 1
        ...
        for idx in range(len(fake_lab)):
            ...
            for i in STdict.keys():
                ...
                if STdict[i]['ischar']:
                    ...
                    viz_pats.append(cv2.addWeighted(STdict[i]['patch'], 0.5, np.ones_like(STdict[i]['patch']) *
                                                    STdict[i]['decoder_attention_score'][idx], 0.5, 0))
